...
bool is_avl_tree(bag_t *bag)
{
    ...
    else if (bag->root->right != NULL && bag->root->left != NULL)
    {
        return is_avl_tree(bag->root->right) && is_avl_tree(bag->root->left);
    }
    else if (bag->root->right == NULL)
    {
        if (bag->root->left->left != NULL || bag->root->left->right != NULL)
        {
            return false;
        }
        else
    ...
    {
        if (bag->root->right->left != NULL || bag->root->right->right != NULL)
        {
            return false;
        }
        else
...
int main()
{
    bag_t *b_avl = bag_create(float_cmp);
    bag_t *b_bst = bag_create(float_cmp);
